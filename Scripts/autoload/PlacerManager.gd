...
@onready var house = preload("res://Scenes/Home1/Home1.tscn")
...
func _input(event: InputEvent) -> void:
	if event is InputEventKey and  event.is_pressed():
		...
		match event.button_index:
			MOUSE_BUTTON_LEFT when can_place:
				placing = false
				can_place = false
				instance.placed()
			MOUSE_BUTTON_LEFT:
...
func _process(delta: float) -> void:
	if placing:
		...
		var ray_end:Vector3 = ray_origin + camera.project_ray_normal(mouse_pos) * interval
